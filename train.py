        ...
        if (idx_epoch + 1)%step==0 or idx_epoch > args.epoch-step:
            with torch.no_grad():
                ...
                for index, test_name in enumerate(test_Names):
                    ...
                    psnr_iter_test, ssim_iter_test, LF_name, _ , _  = test(test_loader, device, net, save_dir)
...
def test(test_loader, device, net, save_dir=None):      
    ...
    for idx_iter, (Lr_SAI_y, Hr_SAI_y, Sr_SAI_cbcr, data_info, LF_name) in tqdm(enumerate(test_loader), total=len(test_loader), ncols=70):
        ...
        subLFout = torch.zeros(numU * numV, 1, args.angRes * args.patch_size_for_test * args.scale_factor,
    ...
    return psnr_iter_test, ssim_iter_test, LF_iter_test, psnr_iter_test_allviews, ssim_iter_test_allviews
