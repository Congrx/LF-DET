...
class get_model(nn.Module):
    def __init__(self, args):
        ...
        for i in range(0,args.depth):
            ...
            self.blocks.append(MixTransfomerBlock(self.channels,args.ang_num_heads,args.spa_num_heads,args.ang_mlp_ratio,args.spa_mlp_ratio,args.spa_trans_num,ang_sr_ratio=args.ang_sr_ratio,spa_sr_ratio=args.spa_sr_ratio,
                                                qkv_bias=True, qk_scale=None, drop=args.drop_rate, attn_drop=args.attn_drop_rate,drop_path=dpr_iter, act_layer=nn.GELU, norm_layer=nn.LayerNorm))
       
        # step3: Hierarchical Feature Aggregation & Upsampling
        self.mla = MLA(self.channels)
        self.upsampling = nn.Sequential(
            nn.Conv2d(self.channels*args.depth//2, self.channels*self.factor ** 2, kernel_size=1, padding=0, dilation=1, bias=False),  
